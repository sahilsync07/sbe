...
const newBodyWebGeneral = `const generatePdfBlob = async (targetBrands) => {
    ...
    for (const group of filteredGroups) {
        for (const product of group.products) {
            ...
            doc.setFontSize(36);
            doc.text(dateStr, 50, 70, { align: "left" });
            doc.text(group.groupName, PAGE_W - 50, 70, { align: "right" });

            doc.setTextColor(255, 255, 255);
            doc.setFont("helvetica", "bold");
            doc.setFontSize(44);
            doc.text("Sri Brundabana Enterprises, Rayagada", PAGE_W / 2, 130, { align: "center" });
            
            doc.setTextColor(255, 255, 255);
            doc.setFont("helvetica", "bold");
            doc.setFontSize(48);
            doc.text(product.productName, PAGE_W / 2, PAGE_H - 120, { align: "center" });
            
            doc.setTextColor(255, 215, 0); // Muted gold, not neon
            doc.setFont("helvetica", "bold");
            doc.setFontSize(42);
            doc.text(\`Qty: \${product.quantity}\`, PAGE_W / 2, PAGE_H - 60, { align: "center" });
        }
    }
