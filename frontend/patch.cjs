...
const endStr = 'return doc.output("blob");\n};';
const endStrAlt = "return doc.output('blob');\n};";
let end = content.indexOf(endStr, start);
let endLength = endStr.length;
if (end === -1) {
    end = content.indexOf(endStrAlt, start);
    endLength = endStrAlt.length;
}
...
content = content.substring(0, start) + newFunc + content.substring(end + endLength);
