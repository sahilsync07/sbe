    ...
    <div class="w-full px-2 sm:px-6 lg:px-8">
      <div class="flex flex-col gap-2">
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-3">
            ...
            >
              ...
              <svg v-else xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 animate-fade-in" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div class="flex flex-col items-center justify-center px-2">
            <h1
              ...
              title="Admin Login"
            >
              <span class="text-blue-600">{{ companyName.split(' ')[0] }}</span>
          ...
          <div class="flex items-center justify-end gap-2">
            ...
            >
              ...
              <div v-else class="flex items-center justify-center w-full h-full animate-fade-in">
                <div v-if="cartTotalItems > 0" class="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full shadow-sm z-10 border border-white">
                  {{ cartTotalItems }}
                </div>
        ...
        <div class="flex items-center gap-2">
          ...
          <!-- Image Toggle -->
          <label
            class="flex items-center cursor-pointer select-none bg-white lg:bg-slate-50 border border-slate-200 rounded-full px-3 py-2 shadow-sm active:scale-95 transition-transform h-[38px]"
            title="Show Images Only"
          >
            <input
              type="checkbox"
              :checked="showImagesOnly"
              @change="$emit('toggle-images', $event.target.checked)"
              class="sr-only"
            />
            <i class="fa-solid fa-image text-blue-600 text-base"></i>
          </label>
        </div>
      </div>
    </div>
