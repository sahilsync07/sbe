  ...
  <div class="min-h-screen w-full bg-slate-50 font-sans text-slate-800">
    ...
    <div class="flex w-full">
      <!-- Side Panel (Bird Eye View) -->
       <!-- Brands View (Replaces Main Body) -->
       <div 
        v-if="store.isSidebarOpen" 
        class="w-full bg-white pb-24 min-h-screen pt-2"
       >
        <div class="px-4 pb-4">
           <div class="flex items-center gap-3 mb-2 py-3 border-b border-slate-100">
             <button 
                @click="store.isSidebarOpen = false"
                class="w-8 h-8 flex items-center justify-center rounded-full bg-slate-100 text-slate-900 hover:bg-slate-200"
                aria-label="Back"
             >
                <i class="fa-solid fa-arrow-left text-sm"></i>
             </button>
             <h2 class="text-xs font-black tracking-widest text-slate-900 uppercase">
                Select Brand
             </h2>
           </div>
            <nav class="space-y-4 px-1">
      ...
      <aside
        class="fixed inset-y-0 right-0 w-full sm:w-80 border-l border-slate-200 z-[60] transform transition-transform duration-300 ease-in-out shadow-2xl flex flex-col bg-white/95 backdrop-blur-sm sm:bg-white pt-safe pb-safe"
        :class="store.isCartOpen ? 'translate-x-0' : 'translate-x-full'"
      >
        <div class="p-4 border-b border-slate-100 flex items-center gap-3 bg-white sticky top-0 z-10">
           <button 
              @click="store.isCartOpen = false"
              class="w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-100 text-slate-500 -ml-2"
              aria-label="Close cart"
           >
              <i class="fa-solid fa-arrow-left text-lg"></i>
           </button>
           <h2 class="text-lg font-bold text-slate-800 flex items-center gap-2">
             Cart <span v-if="cart.length" class="text-sm font-normal text-slate-500">({{ cartTotalItems }})</span>
           </h2>
           <button 
             v-if="cart.length > 0"
             @click="clearCart"
             class="ml-auto w-8 h-8 flex items-center justify-center rounded-full bg-slate-100 text-slate-500 hover:bg-red-100 hover:text-red-600 transition-colors"
             title="Clear Cart"
           >
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <div class="flex items-center justify-between mt-2">
                       <div class="flex items-center gap-2">
                          ...
                          <span class="text-xs font-bold text-slate-800 min-w-[3rem] text-center">{{ item.quantity }} {{ item.quantity > 1 ? 'Pairs' : 'Pair' }}</span>
                          <button @click="updateCartQuantity(index, 1)" class="w-6 h-6 flex items-center justify-center text-white bg-blue-600 hover:bg-blue-700 rounded-full shadow-sm transition-all">+</button>
                       </div>
      ...
      <!-- Main Content -->
      <div v-if="!showLedgerView" class="space-y-6">

        <!-- Mobile View (New Design) -->        <!-- Mobile View (New Design) -->
        <div v-if="store.isMobile" class="pb-24"> 
            <div 
                v-for="(group, index) in filteredStockData"
                :key="group.groupName"
                class="mb-6"
            >
                <div class="px-2 mb-4 mt-2 flex items-center gap-3">
                     <h2 class="text-xl font-black tracking-tight text-slate-900">
                        {{ group.groupName }}
                     </h2>
                     <span class="w-8 h-8 flex items-center justify-center rounded-full bg-blue-50 text-blue-600 text-[11px] font-bold">{{ group.products.length }}</span>
                </div>
                ...
                <div class="grid grid-cols-2 gap-3 px-2">
                    <ProductCard
                        ...
                        :product="{
                            ...
                            image: getOptimizedUrl(product.imageUrl),
                            stock: product.quantity,
                            price: null 
                        }"
                        :is-admin="store.isAdmin"
                        :is-new-arrival="isNewArrival(product)"
                        :cart-quantity="getCartQty(product)"
            ...
            <div v-if="filteredStockData.length === 0" class="flex flex-col items-center justify-center py-20 opacity-50">
                <p>No products found</p>
            </div>
        </div>


        <!-- Desktop View (Legacy Table/Grid) - Only show if NOT mobile -->
        <div v-if="!store.isMobile" class="desktop-view-container pt-8"> 
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-2 sm:gap-4">
                  ...
                  >
                    ...
                    <div class="relative aspect-[3/4] bg-slate-100 overflow-hidden mb-3 rounded-xl border border-slate-100 shadow-inner group-hover:shadow-md transition-shadow">
                      ...
                      <div
                        v-if="store.isAdmin || store.isSuperAdmin"
    ...
    <transition enter-active-class="transition duration-300 ease-out" enter-from-class="translate-y-10 opacity-0" enter-to-class="translate-y-0 opacity-100" leave-active-class="transition duration-200 ease-in" leave-from-class="opacity-100" leave-to-class="opacity-0">
      <button
        v-if="showGoToTop"
        @click="scrollToTop"
        class="fixed bottom-24 right-6 w-14 h-14 flex items-center justify-center bg-slate-900/90 text-white rounded-full shadow-2xl hover:bg-black transition-all hover:-translate-y-1 hover:shadow-black/20 backdrop-blur-sm z-30"
      >
    ...
    <transition enter-active-class="transition duration-300 ease-out" enter-from-class="opacity-0 translate-y-4" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition duration-200 ease-in" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 translate-y-4">
      <div
        v-if="showImagePopup"
        class="fixed inset-0 z-40 top-[60px] bottom-[65px] bg-slate-50 overflow-y-auto overflow-x-hidden flex flex-col"
      >
        <!-- Sticky Navbar -->
...
export default {
  ...
  computed: {
    ...
    filteredStockData() {
      ...
      const query = (store.searchQuery || "").toLowerCase();
       ...
       // Only filter main grid if no pane is open (Legacy logic, maybe keep?)
       // Or just filter if query exists
       if (query && !this.showSidePanel && !this.showCart) {
         filtered = filtered
          .map((group) => ({
            ...group,
      ...
      if (this.selectedGroup === 'All') {
         ...
         this.stockData.forEach(g => {
            g.products.forEach(p => {
                ...
                if (store.searchQuery && !p.productName.toLowerCase().includes(store.searchQuery.toLowerCase())) {
                    return;
                }
  ...
  watch: {
    'store.isSidebarOpen'(newVal) {
        this.showSidePanel = newVal;
        if (newVal) history.pushState({ modal: 'sidebar' }, null);
    },
    'store.isCartOpen'(newVal) {
        if (newVal) history.pushState({ modal: 'cart' }, null);
    },
    showImagePopup(newVal) {
        if (newVal) history.pushState({ modal: 'image' }, null);
    }
  },
  methods: {
    async loadConfig() {
    ...
    // --- History / Back Button Logic for Overlays ---
    handlePopState() {
       // If we receive a popstate (back button), and an overlay is open, close it.
       if (store.isSidebarOpen) {
          store.isSidebarOpen = false;
       } else if (store.isCartOpen) {
          store.isCartOpen = false;
       } else if (this.showImagePopup) {
    ...
    handleSidebarClick(group) {
       this.activeScrollGroup = group.groupName;
       this.scrollToGroup(group.groupName);
       store.isSidebarOpen = false; // Close "view" to show content
    },
    ...
    openSidebar() {
       ...
       // History push handled by watcher
    },
    openCart() {
       this.searchQuery = '';
    ...
    scrollToGroup(groupName, behavior = 'instant') {
      ...
      const performScroll = () => {
         this.$nextTick(() => {
             ...
             if (element) {
                 ...
                 // Update URL without pushState (since we might have just popped state)
                 const url = new URL(window.location);
      ...
      // Close sidebar if open (Mobile behavior)
      if (window.innerWidth < 1024 && store.isSidebarOpen) {
          // Use history.back() to close the sidebar properly (triggering popstate)
          // This keeps history clean and header sync correct
          window.history.back();
          
          // Wait for sidebar to close (popstate listener updates store -> watcher updates showSidePanel)
          setTimeout(() => {
             performScroll();
          }, 100);
      } else {
         // Desktop or sidebar already closed
         performScroll();
      }
    },
