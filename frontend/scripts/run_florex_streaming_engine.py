...
PROGRESS_JSON = r"C:\Projects\sbe\frontend\scripts\florex_live_progress.json"
STREAM_LOG = r"C:\Projects\sbe\frontend\scripts\florex_live_stream.log"
ZIP_PATH = r"C:\Users\Sahil Kumar\Downloads\Pictures\Quick Share\WhatsApp Chat with FLOREX GALLERY.zip"

FLOREX_GROUPS = ['Florex (Swastik)', 'FLOREX', 'SWASTIK']
...
COLOR_MAP = {
    ...
    'PNK': ['PNK', 'PINK', 'ROSE'],
    'MRN': ['MRN', 'MAROON', 'CHRY', 'CHERRY', 'WINE', 'BURGUNDY', 'WNE'],
    'GRN': ['GRN', 'GREEN', 'OLV', 'OLIVE', 'PISTA', 'F_GREEN', 'FGRN', 'MEHANDI', 'MHD', 'MINT'],
    'BLU': ['BLU', 'BLUE', 'NAVY', 'SKY', 'R_BLUE', 'AIRFORCE', 'TURQUOISE', 'AQUA', 'D_SKY', 'P_BLU'],
    'GRY': ['GRY', 'GREY', 'GRAY', 'SLATE', 'D_GREY', 'L_GREY'],
    'WHT': ['WHT', 'WHITE', 'CREAM', 'CRM', 'BEG', 'BGE', 'BEIGE', 'PEANUT', 'PNUT', 'BISCUIT'],
    'YLW': ['YLW', 'YELLOW', 'MUSTARD', 'LEMON'],
    'RED': ['RED', 'CHILI', 'TOMATO'],
    'SLV': ['SLV', 'SILVER'],
    'GLD': ['GLD', 'GOLD', 'GOLDEN'],
    'PCH': ['PCH', 'PEACH'],
    'PPL': ['PPL', 'PURPLE', 'LAVENDER']
}
...
def parse_model_tokens(brand, pname):
    clean = re.sub(r'((?:RS|MRP|@))[\.\s]*(\d+(\.\d+)?)', '', pname, flags=re.IGNORECASE)
    clean = re.sub(r'(?:^|[\s\(])(\d{1,2})\s*[xX*]\s*(\d{1,2})(?:[\s\)]|$)', ' ', clean)
    clean = re.sub(r'\b(FLOREX|SWASTIK|HAMPER)\b', '', clean, flags=re.IGNORECASE)
    tokens = [t.upper() for t in re.split(r'[^A-Za-z0-9]+', clean) if len(t) >= 1]
    filler = {'LDS', 'GENTS', 'KIDS', 'MENS', 'BOYS', 'GIRLS', 'RS', 'MRP', 'CHAPPAL', 'SANDAL', 'SHOES', 'SLIPPER', 'PU', 'EVA', 'AIR', 'NEW', 'F', 'W', '45', '40', '50'}
...
def ocr_matches_color(ocr_text, prod_colors):
    ...
    ot = ocr_text.upper()
    single_clr_m = re.search(r'\b(?:colou?r|clr)[\s\:\-]+([A-Za-z]+)\b', ot)
    if single_clr_m:
...
def upload_bytes_to_cloudinary(img_bytes, filename, folder_path="e-sbe/florex"):
...
async def evaluate_image(z, name, product_index):
    ...
    # Handle explicit article marker if present in OCR
    art_marker_m = re.search(r'ARTICLE(?:\s+NAME)?[\s\:\-]+([A-Za-z0-9\-\s]{3,20})', ocr_text)
    explicit_article = art_marker_m.group(1).strip() if art_marker_m else ""
...
async def main():
    log_event("Starting Dedicated FLOREX / SWASTIK Engine...")
    ...
    for g in stock_data:
        ...
        if any(b.upper() in gname.upper() for b in FLOREX_GROUPS):
    ...
    log_event(f"Loaded {len(product_index)} Florex/Swastik candidate models.")

    if not os.path.exists(ZIP_PATH):
        log_event(f"ERROR: FLOREX zip not found at {ZIP_PATH}")
        return
    ...
    with zipfile.ZipFile(ZIP_PATH, 'r') as z:
        ...
        log_event(f"Loaded {total_imgs} images from FLOREX archive. Beginning batch processing...")
        ...
        for i in range(0, total_imgs, batch_size):
            ...
            for r in res:
                if r:
                    ...
                    if semantic_file in url_cache:
                        ...
                        sec_url = upload_bytes_to_cloudinary(r['img_bytes'], semantic_file, folder_path="e-sbe/florex")
                        if sec_url:
                    ...
                    if sec_url:
                        for prod in r['matched_products']:
                            if not prod.get('secondaryImageUrl'):
                                ...
                                total_uploaded += 1
                                batch_uploaded += 1
                                log_event(f"UPLOADED: {prod['productName']} -> {sec_url}")
            ...
            state["processed"] = min(i + batch_size, total_imgs)
            state["totalUploaded"] = total_uploaded
        ...
        log_event(f"FLOREX ENGINE COMPLETE: Uploaded {total_uploaded} verified products.")
