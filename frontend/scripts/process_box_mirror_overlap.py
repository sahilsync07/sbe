...
REPORT_PATH = r"C:\Projects\sbe\frontend\scripts\box_mirror_overlap_report.json"
...
# Exact overlapping mirror size groups defined by the domain:
# e.g. Ladies standard runs: 4x7, 4x8, 5x8
# Gents standard runs: 6x9, 6x10, 7x10
# Kids runs: 11x13, 1x3, 11x1
MIRROR_SIZE_CLASSES = [
    {'4X7', '4X8', '5X8'},
    {'6X9', '6X10', '7X10'},
    {'11X13', '1X3'},
...
async def process_image_with_mirror_sizes(z, name, product_index):
    try:
    ...
    # Check if all matched candidates belong to the EXACT same brand, core model, and color
    ...
    for c in matched_candidates:
        ...
        if c['group'] != first_group or c['core_key'] != first_core or c_cols != first_cols:
            return None # Collision with different model or color!

    # Now verify that their size groups are STRICTLY mirror-overlapping
    # (e.g. 5x8 and 4x7, or 6x9 and 6x10). If someone is 2x5 and another is 6x9, REJECT!
    sizes = [c['size_str'] for c in matched_candidates if c['size_str']]
    if sizes:
        base_size = sizes[0]
        for s in sizes[1:]:
            if not are_sizes_mirror_overlapping(base_size, s):
                return None # NOT valid mirror sizes (e.g. 2x5 vs 6x9)!
...
async def main():
    print("Executing Strict Mirror-Overlap Box Refinement...", flush=True)
    ...
    with zipfile.ZipFile(zip_path, 'r') as z:
        ...
        print(f"Total images to evaluate: {len(all_names)}", flush=True)

        batch_size = 50
        for i in range(0, len(all_names), batch_size):
            batch = all_names[i:i+batch_size]
            tasks = [process_image_with_mirror_sizes(z, name, product_index) for name in batch]
            ...
            print(f"Mirror Engine: {min(i+batch_size, len(all_names))}/{len(all_names)} evaluated", flush=True)

    with open(FRONTEND_STOCK, 'w', encoding='utf-8') as f:
        json.dump(stock_data, f, indent=2, ensure_ascii=False)

    print(f"\nCompleted Mirror-Overlap Expansion: Attached secondary URLs to {len(all_uploads)} additional mirror variants!", flush=True)
