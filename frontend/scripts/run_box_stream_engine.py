...
async def main():
    log_event("Starting Stream Engine for Box Footwear...")
    ...
    if not zip_files:
        log_event("ERROR: BOX FOOTWEAR archive not found.")
        return
    ...
    with zipfile.ZipFile(zip_files[0], 'r') as z:
        ...
        log_event(f"Loaded {total_imgs} images from archive. Beginning stream batching...")
        ...
        for i in range(0, total_imgs, batch_size):
                    ...
                    if sec_url:
                        for prod in r['matched_products']:
                            if not prod.get('secondaryImageUrl'):
                                ...
                                if len(state["recentlyUploaded"]) > 15:
                                    state["recentlyUploaded"].pop(0)

            # Progressive database checkpoint every 200 images or when uploads happen
            if batch_uploaded > 0 or (i % 200 == 0):
            ...
            state["uploadedThisRun"] = total_new_uploads
        ...
        log_event(f"RUN FINISHED: Attached {total_new_uploads} total verified items.")
