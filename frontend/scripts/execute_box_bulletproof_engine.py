...
SBE_HUB_STOCK = r"C:\Projects\sbe\sbe-hub\assets\stock-data.json"
REPORT_PATH = r"C:\Projects\sbe\frontend\scripts\box_bulletproof_report.json"
...
def upload_bytes_to_cloudinary(img_bytes, filename, folder_path="e-sbe"):
    ...
    try:
        ...
        print(f"Upload error for {filename}: {e}", flush=True)
        return None
...
def is_danger_image_pre_ocr(img):
    w, h = img.size
    ratio = w / h
    # Status card aspect ratio check (< 0.58 indicates tall mobile screenshot)
    if ratio < 0.58:
    ...
    # Header and footer black bars check
    if tb < 40 and bb < 40:
...
async def process_image_bulletproof(z, name, product_index):
    ...
    # Layer 1: Pre-OCR Danger Filter
    pre_flag, pre_reason = is_danger_image_pre_ocr(img)
    if pre_flag:
        return None

    # Layer 2: Fast Local OCR
    try:
    ...
    # Layer 3: Post-OCR Danger Filter
    post_flag, post_reason = is_danger_image_post_ocr(ocr_text)
    if post_flag:
        return None

    # Layer 4: Candidate Article & Color Matching
    matched_candidates = []
    ...
    # Layer 5: Strict Model, Color, and Mirror Size Overlap Verification
    ...
    for c in matched_candidates:
        ...
        if c['group'] != first_group or c['core_key'] != first_core or c_cols != first_cols:
            return None # Rejection: collided with a different model or colorway
    ...
    if sizes:
        ...
        for s in sizes[1:]:
            if not are_sizes_mirror_overlapping(base_size, s):
                return None # Rejection: cross-tier non-mirror jump (e.g. 2x5 vs 6x9)
...
async def main():
    print("=" * 65, flush=True)
    print("BULLETPROOF BOX ENGINE: ZERO DANGER BARS & STRICT MIRROR SIZES", flush=True)
    print("=" * 65, flush=True)
    ...
    print(f"Indexed {len(product_index)} Box Packing candidates.", flush=True)
    ...
    if os.path.exists(SBE_HUB_STOCK):
        with open(SBE_HUB_STOCK, 'w', encoding='utf-8') as f:
            json.dump(stock_data, f, indent=2, ensure_ascii=False)

    with open(REPORT_PATH, 'w', encoding='utf-8') as f:
        json.dump({
            "totalVerifiedUploaded": len(all_uploads),
            "results": all_uploads
        }, f, indent=2)

    print("\n" + "=" * 65, flush=True)
    print(f"EXECUTION COMPLETE: Successfully attached clean photos to {len(all_uploads)} items!", flush=True)
    print("=" * 65, flush=True)
